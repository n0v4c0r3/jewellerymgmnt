...
if(isset($_POST['loginbtn']))
{
	...
	// $urole = $conn->real_escape_string($_POST["userrole"]);
        $sql = "SELECT `id`, `username`, `email`,`password`,`img`,`role` FROM `admin` WHERE (`email` = '{$username}' OR `email`= '{$username}') AND `password` = '{$password}' AND `role` = 1";
        ...
        if($row = mysqli_num_rows($result) > 0)
        {
            ...
            header("LOCATION: /nrmuser/users.php");
            }
            ...
            <div class="col-md-6 bg-white p-5">
                ...
                <div class="form-style">
                    <form method="POST" action="" >
                        ...
                        <div class="pb-2">
                            <button type="submit" class="btn btn-dark w-100 font-weight-bold mt-2" name="loginbtn">Login </button>
                        </div>
                    </form>
